...
const solutions = [
  {
    id: 'tire-dispatch',
    label: 'Tire Dispatch API',
    icon: Cpu,
    title: 'Programmatic Tire Service Orchestration',
    desc: 'Initiate tire change or replacement dispatches programmatically. Connect to active vendor inventories, coordinate technician dispatches, and log policy details automatically.',
    prompt: 'Dispatch Features',
    features: [
      'Automated service request creation',
      'Inventory-aware matching',
      'Technician routing & dispatch',
      'Real-time tire profile ingestion',
      'Secure benefit verification',
      'Unified enterprise logs'
    ],
    cards: [
      {
        title: 'Inventory Sync',
        desc: 'Matches requests to vendors stocking the correct tire size and brand specifications.',
        glow: 'rgba(59,130,246,0.15)'
      },
      {
        title: 'Smart Dispatch',
        desc: 'Routes tire technicians instantly based on geographical proximity and equipment.',
        glow: 'rgba(6,182,212,0.15)'
      },
      {
        title: 'REST Endpoints',
        desc: 'Lightweight JSON endpoints for seamless integration into custom portals.',
        glow: 'rgba(139,92,246,0.15)'
      },
      {
        title: 'Tire Analytics',
        desc: 'Logs wheel, tire style, and vehicle details programmatically.',
  ...
  },
  {
    ...
    icon: Eye,
    title: 'Live Tracking & ETA Streams',
    desc: 'Stream high-precision coordinates from the responding service provider. Share active routes and calculated arrival times dynamically to eliminate customer anxiety.',
    prompt: 'Tracking Features',
    features: [
      'High-precision GPS telemetry',
      'Active route updates stream',
      'Dynamic technician arrival ETAs',
      'Real-time technician details',
      'Milestone status updates',
      'Interactive maps payloads'
    ],
    cards: [
      {
        title: 'Active GPS',
        desc: 'Provides real-time coordinate updates from the en-route technician.',
        glow: 'rgba(245,158,11,0.15)'
      },
      {
        title: 'Route Recalc',
        desc: 'Updates arrival times continuously using current traffic and route progress.',
        glow: 'rgba(6,182,212,0.15)'
      },
      {
        title: 'Milestone Tracking',
        desc: 'Maintains granular tracking across dispatch phases: accepted, en-route, completed.',
        glow: 'rgba(59,130,246,0.15)'
      },
      {
        title: 'Sync Network',
        desc: 'Keeps call centers, dispatch teams, and users updated simultaneously.',
        glow: 'rgba(139,92,246,0.15)'
      }
    ]
  },
  {
    ...
    icon: Activity,
    title: 'Event-Driven Webhook Callbacks',
    desc: 'Connect operational milestones to your platform. Receive secure HTTP POST webhooks immediately when services are requested, updated, or finalized.',
    prompt: 'Webhook Events',
    features: [
      'tire.assistance.requested events',
      'technician.enroute alerts',
      'service.completed webhooks',
      'Secure HMAC signatures checks',
      'Automatic callback retries',
      'Granular metadata payloads'
    ],
    cards: [
      {
        title: 'Instant Alerts',
        desc: 'Notifies your backend immediately upon service state transitions.',
...
export const TireAPI = () => {
  const [activeTab, setActiveTab] = useState('tire-dispatch');
  const activeSolution = solutions.find(s => s.id === activeTab) || solutions[0];
  const { stats } = useTireStats(); // Keep hook to prevent unused variables if imported
    ...
    <section className="py-32 bg-brand-bg/30 relative overflow-hidden border-t border-brand-border">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="max-w-4xl mb-16">
          <h2 className="text-3xl md:text-5xl font-bold text-foreground mb-6">API-First <span className="text-brand-blue">Tire Assistance.</span></h2>
          <p className="text-brand-slate text-lg max-w-3xl">Integrate roadside tire repair, inspection, and replacement logic directly into your fleet software or claims platform.</p>
        </div>
