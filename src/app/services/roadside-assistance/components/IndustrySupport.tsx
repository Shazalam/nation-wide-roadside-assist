    ...
    <section className="py-32 relative z-10 bg-white/[0.01]">
      <div className="container mx-auto px-4">
        <div className="text-center mb-20 max-w-3xl mx-auto">
          <p className="text-[10px] font-bold text-brand-blue uppercase tracking-[0.3em] mb-4">Strategic Outsourcing</p>
          <h2 className="text-4xl lg:text-5xl font-black text-foreground dark:text-white tracking-tight mb-8">Industry Support Solutions</h2>
          <p className="text-brand-slate font-medium text-lg leading-relaxed">
        ...
        <div className="max-w-6xl mx-auto">
            ...
            {industries.map((ind) => (
              <button
                ...
                className={`px-8 py-4 rounded-2xl font-bold text-[11px] uppercase tracking-widest transition-all border ${
                  activeId === ind.id 
                    ? 'bg-brand-blue text-foreground dark:text-white border-brand-blue shadow-[0_0_30px_rgba(47,128,255,0.3)]' 
                    : 'bg-white/5 text-brand-slate border-brand-border hover:bg-white/10 hover:border-brand-border'
                }`}
              >
                {ind.title}
              </button>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <GlassPanel className="p-12 lg:p-20 border-brand-border bg-card/60 shadow-2xl">
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    <div>
                       ...
                       <h3 className="text-3xl lg:text-4xl font-black text-foreground dark:text-white mb-6 tracking-tight">{activeIndustry.title}</h3>
                       <p className="text-lg text-brand-slate leading-relaxed mb-10">
                          {activeIndustry.desc}
                       </p>
                       <Button size="lg" className="bg-brand-blue hover:bg-brand-blue/90 text-foreground dark:text-white font-bold h-14 px-10 rounded-xl group">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       {activeIndustry.features.map((feat, i) => (
                         <div key={i} className="p-6 bg-white/[0.02] border border-brand-border rounded-2xl hover:bg-white/[0.04] transition-all group">
                            <CheckCircle2 className="h-6 w-6 text-emerald-400 mb-6 opacity-40 group-hover:opacity-100 transition-opacity" />
                            <p className="text-sm font-bold text-foreground dark:text-white tracking-tight leading-snug">{feat}</p>
                         </div>
                       ))}
                    </div>
                 </div>
