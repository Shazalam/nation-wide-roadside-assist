      ...
      <div className="container mx-auto px-4">
        <div className="flex flex-col lg:flex-row gap-12 items-end mb-16">
          <div className="max-w-2xl">
            ...
            <h2 className="text-4xl lg:text-6xl font-black text-white mb-8 tracking-tight">Roadside Command Center</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-3 space-y-3">
             ...
             <GlassPanel className="mt-8 p-6 border-brand-orange/20 bg-brand-orange/5">
                ...
                <p className="text-[10px] text-[#94A3B8] uppercase tracking-widest mt-1">High Priority Events</p>
             </GlassPanel>
          </div>
          ...
          <div className="lg:col-span-9">
             <GlassPanel className="border-white/10 bg-[#0A192F]/60 p-0 overflow-hidden shadow-2xl">
                ...
                <div className="px-8 py-6 border-b border-white/5 flex flex-wrap items-center justify-between gap-4 bg-white/[0.01]">
                   <div className="flex items-center gap-6">
                      <div className="flex items-center gap-3">
                         ...
                         <span className="text-xs font-bold text-white tracking-tight">Dispatch Command Center</span>
                      </div>
                      <div className="h-8 w-[1px] bg-white/10" />
                      <p className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest">Active Requests: 1,248</p>
                   </div>
                   <div className="flex items-center gap-3">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-12 h-[600px]">
                   ...
                   <div className="xl:col-span-4 border-r border-white/5 bg-[#081120]/30 overflow-auto">
                      <div className="divide-y divide-white/5">
                         {activeIncidents.map((inc) => (
                           <div key={inc.id} className="p-5 hover:bg-white/[0.02] transition-colors cursor-pointer group">
                              <div className="flex justify-between items-start mb-3">
                                 <p className="text-xs font-black text-white group-hover:text-brand-blue transition-colors">{inc.id}</p>
                                 <Badge variant="outline" className="text-[8px] border-brand-orange/30 text-brand-orange">{inc.status}</Badge>
                              </div>
                              <div className="space-y-1 mb-4">
                                 <p className="text-[10px] font-bold text-white/90">{inc.type}</p>
                                 <div className="flex items-center gap-2">
                                    <MapPin className="h-2.5 w-2.5 text-[#94A3B8]" />
                                    <p className="text-[9px] text-[#94A3B8]">{inc.location}</p>
                                 </div>
                              </div>
                   ...
                   <div className="xl:col-span-8 relative bg-brand-bg/50">
                      ...
                      <div className="absolute inset-0 opacity-20 pointer-events-none" style={{ backgroundImage: 'radial-gradient(#2F80FF 1px, transparent 0)', backgroundSize: '40px 40px' }} />
                      
                      {/* Visualization UI Layers */}
                      <div className="absolute inset-0 p-8 flex flex-col justify-between">
                         <div className="flex justify-between items-start">
                            <GlassPanel className="p-4 bg-brand-navy/90 border-brand-blue/30 w-56 shadow-2xl">
                               <div className="flex justify-between items-center mb-4">
                                  <span className="text-[10px] font-black text-white uppercase tracking-widest">Active Incident</span>
                                  <Badge variant="outline" className="bg-brand-orange/10 border-brand-orange/30 text-brand-orange text-[8px]">En Route</Badge>
                               </div>
                               <h3 className="text-xl font-black text-white mb-4">INC-7849</h3>
                               <div className="space-y-3">
                                  {[
                                    ...
                                    <div key={i} className="flex justify-between text-[9px]">
                                       <span className="text-[#94A3B8] uppercase font-bold">{item.label}</span>
                                       <span className="text-white font-black">{item.val}</span>
                                    </div>
                                  ))}
                               </div>
                               <Button className="w-full mt-6 bg-brand-blue hover:bg-brand-blue/90 text-white font-bold h-10 text-[10px] uppercase tracking-widest">View Details</Button>
                            </GlassPanel>

                            <GlassPanel className="p-6 bg-brand-navy/90 border-white/10 w-56 shadow-2xl">
                               <p className="text-[10px] font-black text-white uppercase tracking-widest mb-6 text-center">Incident Progress</p>
                                  ...
                                  {[
                                    ...
                                    <div key={i} className="flex items-center gap-3 relative">
                                       ...
                                       <div className="flex justify-between items-center w-full">
                                          <span className={`text-[9px] font-bold ${step.status === 'pending' ? 'text-[#94A3B8]' : 'text-white'}`}>{step.label}</span>
                                          <span className="text-[8px] font-mono text-[#94A3B8]">{step.time}</span>
                                       </div>
                                    </div>
                                  ))}
                               </div>
                         ...
                         <div className="grid grid-cols-5 gap-6 mt-8">
                            {[
                              { label: 'Avg Response', val: '32 min', trend: '↑ 9.2%', color: 'text-emerald-400' },
                              { label: 'Requests Today', val: '1,248', trend: '↑ 8.3%', color: 'text-emerald-400' },
                              { label: 'Completed Today', val: '2,381', trend: '↑ 12.4%', color: 'text-emerald-400' },
                              { label: 'Vendor Avail', val: '85%', trend: '↑ 6.7%', color: 'text-emerald-400' },
                              { label: 'First Time Fix', val: '91%', trend: '↑ 4.3%', color: 'text-emerald-400' }
                            ].map((stat, i) => (
                              <div key={i} className="space-y-2 border-t border-white/5 pt-4">
                                 <p className="text-[8px] font-black text-[#94A3B8] uppercase tracking-widest">{stat.label}</p>
