...
export const TrustSection = () => {
  return (
    <section className="py-24 border-y border-white/5 bg-brand-bg/50 relative z-10">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          
          {/* Partners Marquee Area */}
          <div className="lg:col-span-7">
             <p className="text-[10px] font-black text-[#94A3B8] uppercase tracking-[0.4em] mb-12">Trusted by Logistics Leaders</p>
             ...
             <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-10">
                {badges.map((b, i) => (
                  <div key={i} className="flex flex-col items-center text-center gap-3 group">
                     <div className="h-12 w-12 rounded-xl bg-white/5 flex items-center justify-center border border-white/5 group-hover:border-[#2F80FF]/30 transition-all">
                        <b.icon className="h-5 w-5 text-[#94A3B8] group-hover:text-[#2F80FF] transition-colors" />
                     </div>
                     <span className="text-[9px] font-bold text-[#94A3B8] uppercase tracking-widest leading-tight group-hover:text-white transition-colors">{b.label}</span>
                  </div>
                ))}
             </div>
          </div>
