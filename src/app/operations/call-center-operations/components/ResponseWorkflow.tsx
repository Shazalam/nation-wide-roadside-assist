...
import { 
  PhoneCall, ShieldCheck, Cpu, Truck, MapPin, CheckCircle, FileText, ChevronRight
} from 'lucide-react';

const steps = [
  { 
    id: 1, 
    title: "Customer Call Received", 
    description: "Omnichannel intake via phone, web, or app. Instantly creates a new incident profile with caller context.",
    icon: PhoneCall 
  },
  { 
    id: 2, 
    title: "Incident Validation", 
    description: "Automated policy and coverage checks. Validates membership status and exact location coordinates.",
    icon: ShieldCheck 
  },
  { 
    id: 3, 
    title: "AI Dispatch Routing", 
    description: "Machine learning algorithms calculate optimal routes and identify the closest available vendors.",
    icon: Cpu 
  },
  { 
    id: 4, 
    title: "Vendor Assignment", 
    description: "Automated handoff to the selected service provider with all necessary incident telemetry.",
    icon: Truck 
  },
  { 
    id: 5, 
    title: "Live GPS Tracking", 
    description: "Real-time websocket streams track the service vehicle's progress directly to the customer's location.",
    icon: MapPin 
  },
  { 
    id: 6, 
    title: "Service Completion", 
    description: "Vendor arrives on-scene and resolves the issue. Digital signatures and proof of service are logged.",
    icon: CheckCircle 
  },
  { 
    id: 7, 
    title: "Automated Reporting", 
    description: "Incident data is synced to enterprise systems. Automated invoicing and SLA reports are generated.",
    icon: FileText 
  }
];
      ...
      <div className="max-w-[1536px] mx-auto px-6 lg:px-12">
        ...
        <div className="flex flex-col items-center text-center max-w-3xl mx-auto mb-10 lg:mb-12">
           <h2 className="text-4xl lg:text-6xl font-black text-foreground dark:text-white tracking-tighter leading-tight mb-8">
              Incident Response <span className="text-[#2F80FF]">Workflow</span>
           </h2>
           <p className="text-brand-slate font-medium text-lg max-w-2xl mx-auto leading-relaxed">
             Our automated systems streamline the entire roadside response lifecycle, ensuring total visibility, rapid dispatch, and compliance at every stage.
           </p>
        </div>
