...
import { Code, Terminal, Zap, Activity, Cpu, Copy, Check } from 'lucide-react';
...
export const APISection = () => {
  ...
  --url https://api.nationwidetrans.com/v1/fleet/support \\
  --header 'Authorization: Bearer {YOUR_API_KEY}' \\
  --header 'Content-Type: application/json' \\
  --data '{
    "incident_type": "tire_change",
    "vehicle_id": "VHC-78291",
    "location": {
      "lat": 40.7128,
      "lng": -74.0060
    },
...
const support = await client.support.request({
  ...
  priority: 'high'
    ...
    monitor: `// Fleet API Health
{
  "uptime": "99.99%",
  "avg_latency": "186ms",
  "total_requests_24h": 12400,
  "active_integrations": 512
}`
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-5 space-y-8">
            <div className="space-y-4">
              ...
              >
                ...
                <span className="text-[10px] font-black text-[#2F80FF] uppercase tracking-[0.3em]">Fleet API Infrastructure</span>
              </motion.div>
            ...
            <div className="grid grid-cols-2 gap-6">
               <div className="p-4 bg-white/[0.03] border border-brand-border rounded-xl">
                  <p className="text-[9px] font-black text-brand-slate uppercase tracking-widest mb-2">API Requests (24h)</p>
            ...
            <div className="space-y-4">
               ...
               <div className="space-y-2">
                  {[
                    { event: 'fleet.incident.created', time: '2 sec ago' },
                    { event: 'vendor.assigned', time: '8 sec ago' },
                    { event: 'vendor.enroute', time: '1 min ago' },
                    { event: 'incident.resolved', time: '3 min ago' }
                  ].map((e, i) => (
          ...
          <div className="lg:col-span-7">
             <GlassPanel className="p-0 border-brand-border bg-brand-navy shadow-2xl overflow-hidden min-h-[550px] flex flex-col">
                <div className="px-6 py-4 bg-white/[0.02] border-b border-brand-border flex items-center justify-between overflow-x-auto">
                ...
                <div className="px-8 py-4 bg-white/[0.02] border-t border-brand-border flex items-center justify-between">
                   <div className="flex items-center gap-3">
                      <div className="flex -space-x-2">
                        {[1, 2, 3].map(i => (
                          <div key={i} className="h-6 w-6 rounded-full border-2 border-brand-navy bg-brand-bg flex items-center justify-center text-[8px] font-black text-brand-slate">
                            {['JS', 'PY', 'GO'][i-1]}
                          </div>
                        ))}
                      </div>
                      <span className="text-[9px] font-bold text-brand-slate uppercase tracking-widest">Enterprise SDKs Ready</span>
                   </div>
                   <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/30 text-[9px] font-black tracking-widest uppercase">Stable v2.1.0</Badge>
                </div>
             </GlassPanel>
          </div>
