...
export const DispatchWorkflow = () => {
  return (
    <GlassPanel className="p-6 border-white/5 bg-[#0A192F]/40 h-full flex flex-col">
      <h3 className="text-lg font-bold text-white mb-8">Dispatch Operations Workflow</h3>
         ...
         {steps.map(step => (
            <div key={step.id} className="relative z-10 flex flex-col items-center gap-3">
               <div className={`h-12 w-12 rounded-full flex items-center justify-center border transition-colors ${
                  ...
                  <step.icon className={`h-5 w-5 ${step.active ? 'text-white' : ''}`} />
               </div>
               <div className="text-center w-16">
                  <div className="text-[10px] font-bold text-white mb-0.5">{step.id}</div>
                  <div className={`text-[9px] leading-tight ${step.active ? 'text-white font-bold' : 'text-[#94A3B8]'}`}>{step.label}</div>
               </div>
            </div>
         ))}
      </div>
