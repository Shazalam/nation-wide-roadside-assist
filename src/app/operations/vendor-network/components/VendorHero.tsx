...
import {
  Shield, Zap, Globe, Clock, ArrowRight, Activity, ChevronRight
} from 'lucide-react';
import { Button } from '@/components/ui/button';

const MOCK_STATS = {
  activeVendors: 18742,
  activeDispatches: 1243,
};
...
export const VendorHero = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setStats(prev => ({
        activeVendors:    prev.activeVendors    + (Math.random() > 0.5 ? 1 : -1),
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
        <div className="max-w-4xl space-y-10">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 pt-12">
            {[
              { label: 'SLA Compliance',    val: '98.7%',      icon: Shield,   color: 'text-brand-blue'   },
              { label: 'Dispatch Events',   val: '2.8M+',      icon: Zap,      color: 'text-brand-orange' },
              { label: 'Vendor Coverage',   val: '50-State',   icon: Globe,    color: 'text-purple-400'   },
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-3 px-4 py-2 rounded-full bg-brand-blue/10 border border-brand-blue/20 backdrop-blur-md">
            ...
            <span className="text-[9px] font-black text-brand-blue uppercase tracking-widest leading-none">
              {stats.activeVendors.toLocaleString()} Active Vendors
            </span>
          </div>
