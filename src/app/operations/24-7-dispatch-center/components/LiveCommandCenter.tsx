              ...
              {tabs.map(tab => (
                 <button
                   ...
                   className={`flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-all shrink-0 ${
                     activeTab === tab.id 
                     ? 'bg-[#2F80FF]/10 border border-[#2F80FF]/30 text-white' 
                     : 'bg-transparent border border-transparent text-[#94A3B8] hover:bg-white/5 hover:text-white'
                   }`}
                 >
           ...
           <div className="lg:col-span-9 h-[500px]">
              <AnimatePresence mode="wait">
                 <motion.div
                   ...
                   className="h-full"
                 >
                    <GlassPanel className="w-full h-full p-0 border-[#2F80FF]/20 bg-[#081120]/90 relative overflow-hidden flex flex-col">
                       {/* Header */}
                       <div className="px-6 py-4 border-b border-white/10 flex justify-between items-center bg-white/[0.02]">
                          <span className="text-[10px] font-mono text-emerald-400">DATA_STREAM: CONNECTED</span>
                          <span className="text-[10px] font-bold text-white uppercase tracking-widest">{tabs.find(t => t.id === activeTab)?.label}</span>
                       </div>
                       ...
                       <div className="flex-1 p-6 grid grid-cols-2 gap-6 overflow-y-auto">
                          {activeTab === 'active' && (
                             <>
                                <div className="space-y-4">
                                   {[1, 2, 3].map(i => (
                                      <div key={i} className="p-4 rounded-xl border border-white/10 bg-white/5 relative overflow-hidden group">
                                         <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#2F80FF]" />
                                         <div className="flex justify-between items-start mb-2">
                                            <p className="text-[10px] font-bold text-white uppercase tracking-widest">INC-9942-{i}</p>
                                            <span className="text-[9px] font-mono text-[#2F80FF] animate-pulse">EN_ROUTE</span>
                                         </div>
                                         <p className="text-sm text-[#94A3B8] font-medium mb-3">Class 8 Tow • Heavy Duty Winch</p>
                                ...
                                <div className="rounded-xl border border-white/10 overflow-hidden relative">
                                   <div className="absolute inset-0 bg-[#0A192F] opacity-80" />
                          ...
                          {activeTab !== 'active' && (
                             <div className="col-span-2 h-full flex flex-col items-center justify-center opacity-50">
                                ...
                                <p className="text-[10px] font-mono text-white uppercase tracking-widest">Loading Telemetry Stream...</p>
                             </div>
                          )}
                       </div>
                    </GlassPanel>
