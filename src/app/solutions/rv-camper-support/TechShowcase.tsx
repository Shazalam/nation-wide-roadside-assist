...
export default function TechShowcase() {
  ...
  return (
    <section className="py-24 border-y border-[rgba(255,255,255,0.06)] bg-[#0A192F]/20 relative overflow-hidden">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div key={i} className="bg-[#081120] border border-[rgba(255,255,255,0.06)] rounded-xl p-4">
              <div className="text-[10px] text-[#94A3B8] mb-1">{k.label}</div>
              <div className="text-xl font-black text-white font-mono">{k.val}</div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
              ...
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-3 px-5 py-4 rounded-xl text-left transition-all relative border ${isActive
                    ? 'bg-[#2F80FF]/10 border-[#2F80FF]/30 text-white'
                    : 'border-transparent hover:bg-[rgba(255,255,255,0.03)] text-[#94A3B8] hover:border-[rgba(255,255,255,0.05)]'
                  }`}>
                  {isActive && (
                    <motion.div layoutId="techTabBg" className="absolute inset-0 rounded-xl bg-[#2F80FF]/5 border border-[#2F80FF]/20"
                      transition={{ type: 'spring', stiffness: 300, damping: 30 }} />
                  )}
                  <Icon className={`w-4 h-4 relative z-10 ${isActive ? 'text-[#2F80FF]' : 'text-[#94A3B8]'}`} />
          ...
          {/* Dashboard panel */}
          <div className="flex-1 bg-[#081120] border border-[rgba(255,255,255,0.07)] rounded-2xl overflow-hidden shadow-2xl min-h-[480px] relative">
              ...
              {activeTab === 'telemetry' && (
                <motion.div key="telemetry" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="p-6 h-full flex flex-col gap-5">
                  ...
                  <div className="flex-1 grid grid-cols-5 gap-4">
                    {/* Map area */}
                    <div className="col-span-3 bg-[#0A192F] rounded-xl border border-[rgba(255,255,255,0.05)] relative overflow-hidden flex items-center justify-center">
                      ...
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
                        ...
                        <div className="mt-1 bg-[#0A192F] border border-[#FF7A1A]/30 rounded px-2 py-0.5">
                          <span className="text-[8px] font-mono text-[#FF7A1A]">RV-7821 · Yellowstone NP</span>
                        </div>
                        <span className="text-[8px] font-mono text-[#94A3B8] mt-0.5">ETA 35 min</span>
                      </div>
                      ...
                      <div className="absolute bottom-1/4 right-1/4 flex flex-col items-center">
                        ...
                        <span className="text-[8px] font-mono mt-0.5 text-[#94A3B8]">UNIT 17 · 45m</span>
                      </div>
                    ...
                    {/* Service categories */}
                    <div className="col-span-2 bg-[#0A192F] rounded-xl border border-[rgba(255,255,255,0.05)] p-4">
                      <div className="text-[9px] font-mono text-[#94A3B8] uppercase tracking-widest mb-4">Service Categories</div>
                      <div className="space-y-3">
                        {serviceCategories.map((s) => (
                          <div key={s.label}>
                            <div className="flex justify-between text-xs mb-1">
                              <span className="text-[#94A3B8]">{s.label}</span>
                              <span className="font-mono text-white">{s.count.toLocaleString()}</span>
                            </div>
                            <div className="w-full h-1 bg-[#081120] rounded-full overflow-hidden">
              ...
              {activeTab === 'dispatch' && (
                <motion.div key="dispatch" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="p-6 flex flex-col gap-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-[#0A192F] rounded-xl border border-[rgba(255,255,255,0.05)] p-5 space-y-4">
                      {[{ l: 'SLA Compliance', v: 99.2, c: '#10b981' }, { l: 'Auto-Dispatch Rate', v: 87.5, c: '#2F80FF' }, { l: 'Vendor Match Quality', v: 94.1, c: '#FF7A1A' }].map(m => (
                        <div key={m.l}>
                          ...
                          <div className="h-2 w-full bg-[#081120] rounded-full overflow-hidden">
                    ...
                    <div className="bg-[#0A192F] rounded-xl border border-[rgba(255,255,255,0.05)] p-5">
                      <div className="text-[9px] font-mono text-[#94A3B8] uppercase tracking-widest mb-4">Recent Automations</div>
                      <div className="space-y-3">
                        {[{ id: 8041, type: 'Heavy Duty Tow', t: '2s ago' }, { id: 8042, type: 'Mobile Tech Dispatch', t: '8s ago' }, { id: 8043, type: 'Vendor Matched: Tire', t: '18s ago' }].map(d => (
                          <div key={d.id} className="flex gap-3 items-start border-l-2 border-[#2F80FF] pl-3 py-1">
                            ...
                            <div>
                              <div className="text-sm font-medium text-white">{d.type}</div>
                              <div className="text-[9px] font-mono text-[#94A3B8]">ID: DISP-{d.id} · {d.t}</div>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                <motion.div key="predictive" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="p-6 flex flex-col gap-5">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {[{ label: 'National Avg ETA', val: '24m', sub: 'All vehicle types', color: '#2F80FF' }, { label: 'vs Last Quarter', val: '-12%', sub: 'Improvement trend', color: '#10b981' }, { label: 'Prediction Accuracy', val: '94.3%', sub: 'ML model v3.1', color: '#FF7A1A' }].map(k => (
                      <div key={k.label} className="bg-[#0A192F] rounded-xl border border-[rgba(255,255,255,0.05)] p-5">
                        <div className="text-[10px] text-[#94A3B8] mb-2">{k.label}</div>
                        <div className="text-3xl font-black font-mono" style={{ color: k.color }}>{k.val}</div>
                        <div className="text-[10px] text-[#94A3B8] mt-1">{k.sub}</div>
                      </div>
                    ))}
                  </div>
                  <div className="flex-1 bg-[#0A192F] rounded-xl border border-[rgba(255,255,255,0.05)] p-5 relative overflow-hidden">
                    <div className="text-[9px] font-mono text-[#94A3B8] uppercase tracking-widest mb-3">30-Day ETA Trend</div>
                    ...
                    <div className="relative z-10 mt-auto pt-24 flex gap-8">
                      <div className="flex items-center gap-2 text-[10px] font-mono">
                        ...
                        <span className="text-[#94A3B8]">ETA Trend</span>
                      </div>
                      <div className="flex items-center gap-2 text-[10px] font-mono">
                        <Clock className="w-3 h-3 text-[#2F80FF]" />
                        <span className="text-[#94A3B8]">Updated every 60s</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
