    ...
    <section className="py-32 relative z-10 bg-brand-bg/30">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h2 className="text-4xl lg:text-5xl font-black text-white mb-6">Built for Every Fleet</h2>
        </div>
              ...
              <button
                ...
                className={`w-full flex items-center gap-5 px-8 py-7 rounded-3xl transition-all relative overflow-hidden text-left ${
                  activeTab === ind.id 
                    ...
                    : 'bg-white/[0.02] border-white/5 border hover:bg-white/[0.04]'
                }`}
              >
                <div className={`h-12 w-12 rounded-2xl flex items-center justify-center ${activeTab === ind.id ? 'bg-brand-blue text-white' : 'bg-white/5 text-brand-slate'}`}>
                   <ind.icon className="h-6 w-6" />
                </div>
                <div>
                   <h4 className={`text-lg font-bold tracking-tight ${activeTab === ind.id ? 'text-white' : 'text-brand-slate'}`}>{ind.label}</h4>
            ...
            <AnimatePresence mode="wait">
              {industries.map((ind) => ind.id === activeTab && (
                <motion.div
                  ...
                  className="h-full"
                >
                  <GlassPanel className="p-10 lg:p-16 h-full border-white/10 bg-[#0A192F]/40 relative overflow-hidden">
                    {/* Background Glow */}
                    ...
                    <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-12 items-center h-full">
                       <div className="space-y-8">
                          <h3 className="text-3xl lg:text-4xl font-black text-white leading-tight">{ind.label} Dispatch Operations</h3>
                          ...
                          <Button className="bg-brand-blue text-white font-bold h-12 px-8 rounded-xl">Download Sector Report</Button>
                       </div>
                       ...
                       <div className="flex flex-col gap-6">
                          <GlassPanel className="p-8 bg-brand-navy/90 border-brand-blue/20">
                             ...
                             <p className="text-3xl font-black text-white">{ind.kpi}</p>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                             {[
                               ...
                               <GlassPanel key={i} className="p-5 bg-white/[0.02] border-white/5">
