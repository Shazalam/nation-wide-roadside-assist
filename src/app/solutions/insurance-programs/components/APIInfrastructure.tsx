        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          ...
          <div className="space-y-10">
            <div>
              ...
              <h2 className="text-4xl lg:text-6xl font-black tracking-tight text-foreground dark:text-white mb-8">
                API-First <br />
                <span className="text-[#2F80FF]">Infrastructure.</span>
              </h2>
              <p className="text-brand-slate text-xl leading-relaxed max-w-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {[
                ...
                <div key={i} className="flex gap-4">
                  ...
                  <div>
                    <div className="text-sm font-bold text-foreground dark:text-white mb-1 uppercase tracking-widest">{item.title}</div>
                    <div className="text-[11px] text-brand-slate leading-normal">{item.desc}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 pt-4">
              <button className="w-full sm:w-auto px-8 py-4 bg-[#2F80FF] text-foreground dark:text-white rounded-2xl font-bold text-[11px] uppercase tracking-widest hover:shadow-[0_0_30px_rgba(47,128,255,0.4)] transition-all flex items-center justify-center gap-3">
                Explore API Docs <ArrowRight className="w-4 h-4" />
              </button>
              <button className="w-full sm:w-auto px-8 py-4 bg-white/5 border border-brand-border text-foreground dark:text-white rounded-2xl font-bold text-[11px] uppercase tracking-widest hover:bg-white/10 transition-all">
                Request API Key
              </button>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <div className="relative bg-brand-bg rounded-[2.5rem] border border-brand-border shadow-[0_50px_100px_rgba(0,0,0,0.8)] overflow-hidden">
              {/* Terminal Header */}
              <div className="bg-card px-4 md:px-8 py-4 md:py-6 border-b border-brand-border flex items-center justify-between">
                <div className="flex items-center gap-4 md:gap-8 overflow-x-auto no-scrollbar pr-4">
                  {(['dispatch', 'claims', 'webhooks'] as const).map(t => (
                    <button
                      ...
                      className={`text-[11px] font-mono uppercase tracking-widest font-bold transition-colors relative ${activeTab === t ? 'text-[#2F80FF]' : 'text-brand-slate hover:text-foreground dark:text-white'
                        }`}
                    >
                      {t}.json
                      {activeTab === t && (
                ...
                <button
                  onClick={handleCopy}
                  className="p-3 hover:bg-white/5 rounded-xl transition-colors text-brand-slate hover:text-foreground dark:text-white"
                >
              ...
              {/* Code Area */}
              <div className="p-6 md:p-10 h-[350px] md:h-[450px] overflow-auto font-mono text-[12px] md:text-[14px] leading-relaxed no-scrollbar bg-brand-bg">
              ...
              {/* Terminal Footer */}
              <div className="bg-card/50 px-4 md:px-8 py-4 border-t border-brand-border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                  <span className="text-[10px] font-mono text-brand-slate uppercase tracking-widest font-bold">API v1.4.2 // Latency: 24ms</span>
                </div>
                <div className="flex gap-4">
                  ...
                  <div className="text-[10px] font-mono text-foreground dark:text-white/20 uppercase font-bold">SHA: 8k2l3m4</div>
                </div>
              </div>
            </div>
            ...
            <motion.div
              ...
              className="absolute top-20 -right-20 bg-card/90 backdrop-blur-3xl border border-brand-border p-6 rounded-3xl w-72 shadow-2xl z-20 hidden xl:block"
            >
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between group cursor-pointer">
                    <span className="text-[10px] font-mono text-brand-slate group-hover:text-foreground dark:text-white transition-colors">{evt}</span>
                    <ChevronRight className="w-3 h-3 text-foreground dark:text-white/10 group-hover:text-[#2F80FF] transition-all" />
                  </div>
                ))}
              </div>
